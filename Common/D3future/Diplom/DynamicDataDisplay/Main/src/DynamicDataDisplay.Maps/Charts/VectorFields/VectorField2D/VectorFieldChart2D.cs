...
{
	public class VectorFieldChart2D : VectorFieldChartBase
	{
		...
		public VectorFieldChart2D()
		{
			ViewportPanel.SetViewportBounds(viewport3D, new DataRect(0, 0, 1, 1));

			var camera = new PerspectiveCamera(new Point3D(100, 100, 400), new Vector3D(0, 0, -1), new Vector3D(0, 1, 0), 45);
		...
		{
			VectorFieldChart2D owner = (VectorFieldChart2D)d;
			owner.RebuildUI();
		}
