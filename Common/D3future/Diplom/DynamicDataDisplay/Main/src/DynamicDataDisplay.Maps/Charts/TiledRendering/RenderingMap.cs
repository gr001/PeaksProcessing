...
namespace Microsoft.Research.DynamicDataDisplay.Maps.Charts.TiledRendering
{
	...
	public class RenderingMap : Map
	{
		...
		public int LevelSubdivision
		{
			get { return (int)GetValue(LevelSubdivisionProperty); }
			set { SetValue(LevelSubdivisionProperty, value); }
		}

		public static readonly DependencyProperty LevelSubdivisionProperty = DependencyProperty.Register(
		  "LevelSubdivision",
		  typeof(int),
		  typeof(RenderingMap),
		...
		protected override void OnPlotterAttached(Plotter plotter)
		{
			...
			// are not relevant
			Plotter.Viewport.Constraints.Remove(base.ProportionsConstraint);
			Plotter.Viewport.Constraints.Remove(base.MaxSizeConstraint);
		}
		...
		protected override void UpdateLevel(CoordinateTransform transform)
		{
			...
			var num = output.Height / TileSize;
		...
		{
			var levelSubdivision = LevelSubdivision;
			double result = Math.Round(level * levelSubdivision) / levelSubdivision;
			return result;
		}
