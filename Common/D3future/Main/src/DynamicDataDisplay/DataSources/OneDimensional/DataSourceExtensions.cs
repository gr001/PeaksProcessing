...
namespace Microsoft.Research.DynamicDataDisplay.DataSources
{
	public static class DataSourceExtensions
	{
        ...
        public static EnumerableDataSource<T> AsXDataSource<T>(this IEnumerable<T> collection, T scale)
        {
            if (typeof(T) == typeof(double))
            {
                return ((IEnumerable<double>)collection).AsXDataSource((scale as double?).Value) as EnumerableDataSource<T>;
            }
            else if (typeof(T) == typeof(float))
            {
                return ((IEnumerable<float>)collection).AsXDataSource((scale as float?).Value) as EnumerableDataSource<T>;
            }

            throw new InvalidOperationException("Cannot create data source for the given data!");
        ...
        public static EnumerableDataSource<double> AsXDataSource(this IEnumerable<double> collection, double scale)
        {
            EnumerableXDataSource<double> ds = new EnumerableXDataSource<double>(collection);
            ds.SetXMapping(x => x * scale);
            return ds;
        }
