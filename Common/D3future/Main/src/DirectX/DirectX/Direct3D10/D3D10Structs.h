...
public:
    ...
    property ReadOnlyCollection<Boolean>^ BlendEnable
    {
        ReadOnlyCollection<Boolean>^ get()
        {
            ...
            {
                blendEnable = gcnew cli::array<Boolean>(BlendEnableArrayLength);
            }
        ...
        ReadOnlyCollection<ColorWriteEnableComponents>^ get()
        {
            if (renderTargetWriteMask == nullptr)
            {
                renderTargetWriteMask =
                    gcnew cli::array<ColorWriteEnableComponents>(RenderTargetWriteMaskArrayLength);
...
private:
    ...
    cli::array<ColorWriteEnableComponents>^ renderTargetWriteMask;
    cli::array<Boolean>^ blendEnable;
...
internal:
    BlendDescription(const D3D10_BLEND_DESC & blendDescription)
    {
        ...
        renderTargetWriteMask = gcnew cli::array<ColorWriteEnableComponents>(RenderTargetWriteMaskArrayLength);
...
public:
    ...
    property ReadOnlyCollection<RenderTargetBlendDescription1>^ RenderTarget
    {
        ReadOnlyCollection<RenderTargetBlendDescription1>^ get()
        {
            ...
            {
                renderTarget  = gcnew cli::array<RenderTargetBlendDescription1>(RenderTargetArrayLength);
            }
...
private:
    ...
    cli::array<RenderTargetBlendDescription1>^ renderTarget;
};
...
public:
    ...
    property ReadOnlyCollection<Single>^ BlendFactor
    {
        ReadOnlyCollection<Single>^ get()
        {
            ...
            {
                 blendFactor = gcnew cli::array<Single>(BlendFactorArrayLength);
            }
...
private:
    ...
    cli::array<unsigned char>^ defaultValue;
...
internal:
    ShaderVariableDescription(const D3D10_SHADER_VARIABLE_DESC & desc)
    {
        ...
        if (desc.Size > 0)
        {
            defaultValue = gcnew cli::array<unsigned char>(desc.Size);
