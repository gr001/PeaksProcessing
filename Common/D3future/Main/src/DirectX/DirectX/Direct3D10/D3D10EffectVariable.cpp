...
cli::array<unsigned char>^ EffectVariable::GetRawValue(UInt32 count)
{
    if (count == 0)
    {
        return nullptr;
    ...
    cli::array<unsigned char>^ data = gcnew cli::array<unsigned char>(count);
...
void EffectVariable::SetRawValue(cli::array<unsigned char>^ data)
{
