...
IEnumerable<DirectWrite::LineMetrics>^ TextLayout::LineMetrics::get(void)
{
    ...
    {
        cli::array<DirectWrite::LineMetrics>^ metricsArray = gcnew cli::array<DirectWrite::LineMetrics>(actualLineCount);
...
IEnumerable<DirectWrite::ClusterMetrics>^ TextLayout::ClusterMetrics::get(void)
{
    ...
    if (actualClusterCount > 0)
    {
        ...
        cli::array<DirectWrite::ClusterMetrics>^ metricsArray = gcnew cli::array<DirectWrite::ClusterMetrics>(actualClusterCount);
...
cli::array<HitTestMetrics>^ TextLayout::HitTestTextRange(
    UINT32 textPosition,
    UINT32 textLength,
    FLOAT originX,
    FLOAT originY
    )
{
    ...
    if (actualCount > 0)
    {
        cli::array<HitTestMetrics>^ metricsArray = gcnew cli::array<HitTestMetrics>(actualCount);
