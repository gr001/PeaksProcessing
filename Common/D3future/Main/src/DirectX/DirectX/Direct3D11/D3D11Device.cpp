...
D3DDevice^ D3DDevice::CreateDevice(Adapter^ adapter, Direct3D11::DriverType driverType, String^ softwareRasterizerLibrary, Direct3D11::CreateDeviceOptions options, cli::array<FeatureLevel>^ featureLevels)
{
    PFN_D3D11_CREATE_DEVICE createFuncPtr = 
        (PFN_D3D11_CREATE_DEVICE)LibraryLoader::Instance()->GetFunctionFromDll(
            D3D11Library, "D3D11CreateDevice");
...
D3DBuffer^ D3DDevice::CreateBuffer(BufferDescription description, ... cli::array<SubresourceData>^ initialData)
{
...
D3DBuffer^ D3DDevice::CreateBuffer(BufferDescription description)
{
	...
	D3D11_BUFFER_DESC* descr = (D3D11_BUFFER_DESC*)ptr;
    Validate::VerifyResult(CastInterface<ID3D11Device>()->CreateBuffer(
...
{
    cli::array<unsigned char>^ dataArray = CommonUtils::ReadStream(stream);
...
GeometryShader^ D3DDevice::CreateGeometryShaderWithStreamOutput(IntPtr shaderBytecode, UInt32 shaderBytecodeLength, IEnumerable<StreamOutputDeclarationEntry>^ SODeclarations, 
    cli::array<UInt32>^ bufferStrides, UInt32 rasterizedStream, ClassLinkage^ classLinkage)
{
...
GeometryShader^ D3DDevice::CreateGeometryShaderWithStreamOutput(IntPtr shaderBytecode, UInt32 shaderBytecodeLength, IEnumerable<StreamOutputDeclarationEntry>^ SODeclarations, 
    cli::array<UInt32>^ bufferStrides, UInt32 rasterizedStream)
{
...
Texture1D^ D3DDevice::CreateTexture1D(Texture1DDescription description, ... cli::array<SubresourceData>^ initialData)
{
    if (initialData == nullptr || initialData->Length == 0)
    {
...
Texture2D^ D3DDevice::CreateTexture2D(Texture2DDescription description, ... cli::array<SubresourceData>^ initialData)
{
    if (initialData == nullptr || initialData->Length == 0)
    {
...
Texture3D^ D3DDevice::CreateTexture3D(Texture3DDescription description, ... cli::array<SubresourceData>^ initialData)
{
    if (initialData == nullptr || initialData->Length == 0)
    {
