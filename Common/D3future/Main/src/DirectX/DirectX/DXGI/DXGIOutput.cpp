...
cli::array<ModeDescription>^ Output::GetDisplayModeList(Format colorFormat, NonDefaultModes modes)
{
    UINT numModes = GetNumberOfDisplayModes(colorFormat, modes);
    cli::array<ModeDescription>^ descriptions = nullptr;

    if (numModes > 0)
    {
        descriptions = gcnew cli::array<ModeDescription>(numModes);
